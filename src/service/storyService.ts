...
export class StoryService {
    ...
    public async getStoryCards(key: string): Promise<CardWebModel[]> {
        let jqlResponse = await this.jqlService.doRequest(this.jqlService.prepareJqlRequest(
            this.JQL_REQUEST + key, this.JQL_FIELDS));
        return new Promise<CardWebModel[]>((resolve) => {
            resolve(this.transformUtils.transform(jqlResponse.issues[0].fields.subtasks,
